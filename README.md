...
- **收藏**：整库同步到 `我的收藏/`；无扫描水位，只补缺失（`skip_same` + `media_index` / 磁盘去重）
- **频道**：已加入对话 + **自定义频道**（`@名称` 或频道 ID，未加入的公开频道）；列表看覆盖进度，可同步最新消息；详情「继续下载」按批扫描；可调扫描水位；每批条数持久化（默认 100）；失败重试只下失败项、不推进水位；自定义频道可删除
- **任务**：粘贴 `t.me` 链接入队；单项删除 / 清除已完成；进度 SSE（与收藏 / 频道详情共用单路连接）
- **监听**：按间隔轮询已监听频道 / 群 / 收藏 / 自定义频道，增量自动入队；可按内容类型（全部 / 媒体 / 图片 / 视频）筛选；添加时同步当前最新消息 ID 作为水位
- **资源库**：按频道 / 类型浏览、预览；扫盘只维护索引，**不改**频道或收藏水位
- **设置**：文件名模板、并发与线程、相册整组、Takeout、代理、无图模式、监听间隔等
